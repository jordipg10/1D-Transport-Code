! Writes data and results of 1D transient transport model
subroutine write_transport_1D_transient(this,Time_out,output)
    ...
    !select type (this%tpt_props_heterog%)
    !type is (tpt_this%tpt_props_heterog%_homog_c)
    ...
    !type is (tpt_this%tpt_props_heterog%_heterog_c)
        !write(1,*) this%tpt_props_heterog%%flux
    !end select
    ...
    !write(1,"(2x,'r:'/)")
    !do i=1,this%spatial_discr%Num_targets-this%spatial_discr%targets_flag
    !    write(1,"(2x,F15.5)") this%this%tpt_props_heterog%%source_term(i)
    !end do
