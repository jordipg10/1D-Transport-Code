subroutine solve_PDE_EE_Delta_t_homog(this,Time_out,output)
    ! Solves 1D transient PDE with homogeneous time step using Euler explicit method 
    ...
    type is (transport_1D_transient_c)
        ...
        ! Explicit Euler
            ...
            ! Mass balance error
                sq_mass_bal_err=p_mass_bal_err(this,conc_old,conc_new,time_discr%Delta_t,Delta_x)**2
                cum_sq_mass_bal_err=cum_sq_mass_bal_err + sq_mass_bal_err
                if (abs(Time-Time_out(icol))<epsilon) then
